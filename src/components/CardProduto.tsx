...
export default function CardProduto({ product, onOpen, onAdd }: CardProdutoProps) {
  const pricedSizes = ((product as Record<string, unknown>).sizes as Array<{ price?: number | string }> || []).filter((s) => Number(s.price) > 0);
  const hasSizes = pricedSizes.length > 0;
  const minPrice = hasSizes
    ? Math.min(...pricedSizes.map((s) => Number(s.price)), Infinity)
