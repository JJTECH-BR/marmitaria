...
import { FRIES_OPTIONS, PREMIUM_MEATS } from "../constants/menu";
import type { SizeOption, PremiumMeat } from "../constants/menu";
...
export default function ProductModal({ product, isOpen, onClose, onConfirm }: ProductModalProps) {
  ...
  const productSides = productRecord?.sides as string[] | undefined;
...
>>>>>>> master:src/components/ProductModal.tsx
  ...
  useEffect(() => {
    if (isOpen && product) {
      const sizes = (product as Record<string, unknown>).sizes as SizeOption[] | undefined;
      const firstSize = (sizes || []).find((s) => Number(s.price) > 0) || null;
...
>>>>>>> master:src/components/ProductModal.tsx
  ...
  const size = useMemo(() => {
    ...
    return sizes?.find((s) => s.value === sizeValue) || null;
  }, [productRecord, sizeValue]);

  const pricedSizes = useMemo(() => {
    const sizes = productRecord?.sizes as SizeOption[] | undefined;
    return (sizes || []).filter((s) => Number(s.price) > 0);
  }, [productRecord]);
...
function OptionGroup({ title, subtitle, required, children }: OptionGroupProps) {
  return (
...
function SizeCard({ option, active, onClick }: SizeCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function ChoiceCard({ label, active, onClick }: ChoiceCardProps) {
  return (
    <button
      type="button"
      onClick={onClick}
...
function Pill({ active, disabled, onClick, children }: PillProps) {
  return (
    <button
      type="button"
      onClick={onClick}
