...
interface CartItemDetail {
  ...
  proteins?: string[];
  fries?: { id: string; name: string } | string | null;
  sides?: string[];
...
export default function CarrinhoPage() {
  ...
  const { items, note, setNote, updateQuantity, removeItem, subtotal, total, clearCart } =
    useCart();
  const { canPlaceOrder, incrementOrderCount } = useDemo();
  ...
  const handleFinish = () => {
    if (!canPlaceOrder) {
      toast.error("Limite de pedidos do demo atingido. Não é possível fazer mais pedidos.");
      return;
    }
    if (!canFinalize) {
    ...
    const orderData = {
      items: items as unknown as Parameters<typeof buildOrderMessage>[0]["items"],
      note,
      total,
      settings,
      categories: categories as unknown as Parameters<typeof buildOrderMessage>[0]["categories"],
      customer,
      delivery: {
        type: deliveryOption,
        ...
        <aside className="h-fit space-y-6 rounded-3xl bg-card p-5 shadow-card lg:sticky lg:top-24">
          ...
          <div className="space-y-4">
            ...
            <Input
              ...
              value={customer.name}
              onChange={(event: React.ChangeEvent<HTMLInputElement>) => setCustomer((c) => ({ ...c, name: event.target.value }))}
              required
            />
            <Input
              id="telefone"
              ...
              onChange={(event: React.ChangeEvent<HTMLInputElement>) => setCustomer((c) => ({ ...c, phone: event.target.value }))}
              required
            />
          </div>
...
function ItemDetails({ item }: { item: CartItemDetail }) {
  const rows: string[] = [];
