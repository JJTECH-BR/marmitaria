...
export default function HomePage() {
  ...
  const quickAdd = (product: Product) => {
    if (count >= maxCartItems) {
      toast.error(`Limite de ${maxCartItems} itens no carrinho. Remova algo para adicionar.`);
      return;
    }
    const isCustomizable = (product as Record<string, unknown>).sizes || (product as Record<string, unknown>).proteins || (product as Record<string, unknown>).meats;
    if (isCustomizable) {
      openProduct(product);
      return;
    }
  ...
  const handleConfirm = (
    ...
    if (count + quantity > maxCartItems) {
      toast.error(`Limite de ${maxCartItems} itens no carrinho. Remova algo para adicionar.`);
      return;
    }
    addItem(product, quantity, note, customization as Parameters<typeof addItem>[3]);
    toast.success(`${product.name} adicionado ao pedido`);
    setSelected(null);
  };
...
function Hero({ company }: { company: { name: string; banner?: string } | null }) {
  return (
...
function MenuSection({ id, title, subtitle, products, onOpen, onAdd }: MenuSectionProps) {
  return (
...
function Footer({ company }: { company: { name: string; logo?: string; address?: string; schedule?: string; phone?: string; whatsapp?: string } | null }) {
  return (
